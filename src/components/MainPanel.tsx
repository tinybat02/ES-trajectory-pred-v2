...
export class MainPanel extends PureComponent<Props> {
  ...
  componentDidUpdate(prevProps: Props, prevState: State) {
    if (prevProps.data.series[0] !== this.props.data.series[0]) {
      ...
      this.setState({ ...this.state, options: [], current: 'None' });
    ...
    if (prevState.current !== this.state.current) {
      ...
      if (this.state.current !== 'None') {
        ...
        for (let i = 0; i < routeData.length; i++) {
          totalPoints.push(createPoint(routeData, i, floorData));
        }
        ...
        const firstPoint = createPoint(routeData, 0, floorData);
        pointFeatures.push(firstPoint);
        if (routeData.length > 1) {
          const secondPoint = createPoint(routeData, 1, floorData);
  ...
  handleIterRoute = (type: string) => () => {
    ...
    if (type === 'previous' && iterRoute > 0) {
      ...
      this.setState({ iterRoute: iterRoute - 1 }, () => {
        ...
        const beginPoint = createPoint(routeData, this.state.iterRoute, floorData);
        const endPoint = createPoint(routeData, this.state.iterRoute + 1, floorData);
